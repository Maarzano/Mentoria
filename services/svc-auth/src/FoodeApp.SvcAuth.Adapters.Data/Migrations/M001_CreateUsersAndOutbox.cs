...
namespace FoodeApp.SvcAuth.Adapters.Data.Migrations;

[Migration(1, "Create auth schema, users table and outbox_messages table")]
public sealed class M001_CreateUsersAndOutbox : Migration
{
    public override void Up()
    {
        ...
        Execute.Sql("ALTER TABLE auth.users ADD CONSTRAINT chk_role CHECK (role IN ('comprador', 'lojista'));");

        Create.Table("outbox_messages").InSchema("auth")
    ...
    {
        Delete.Table("outbox_messages").InSchema("auth");
        Delete.Table("users").InSchema("auth");
        Execute.Sql("DROP SCHEMA IF EXISTS auth;");
    }
}
